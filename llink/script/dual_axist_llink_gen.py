...
def parse_config_file(cfgfile):
    ...
    for line_no, line in enumerate(cf):
        ...
        #print("Key {} value {} llink_name {}".format(key,value,linkname))
        if key == '}' and linkname == "ST_M2S": ## End of a Logic LInk
             
            ## Create enables as the last entry of rep struct LL
            if configuration['REPLICATED_STRUCT']:
                if logiclink['HASVALID']:
                    onesignal = {'NAME':"user_m2s_enable",
                                 'DIR':logiclink['DIR'],
                                 'TYPE':'rstruct_enable',
                                 'SIGWID':1,
                                 'MSB':0,
            ...
            continue
        #hpr: ------------------------------------------------------------------------------------
        if key == '}' and linkname == "ST_S2M": ## End of a Logic LInk
             
            ## Create enables as the last entry of rep struct LL
            if configuration['REPLICATED_STRUCT']:
                if logiclink['HASVALID']:
                    onesignal = {'NAME':"user_s2m_enable",
                                 'DIR':logiclink['DIR'],
                                 'TYPE':'rstruct_enable',
                                 'SIGWID':1,
                                 'MSB':0,
...
def calc_raw_1phydata(configuration, mux_mode, enable, use_cfg_rate):
    ...
    if enable :
        if mux_mode == 'RSTRUCT':
            ...
            tx_rate = dual_galt.covert_rate_gen2_as_gen1(configuration['TX_RATE'])
            rx_rate = dual_galt.covert_rate_gen2_as_gen1(configuration['RX_RATE'])
...
def calculate_bit_locations(configuration):
    ...
    if configuration['TX_ENABLE_PACKETIZATION']:
        ...
        configuration = dual_galt.calculate_bit_loc_galt(True, configuration)
    ...
    if configuration['RX_ENABLE_PACKETIZATION']:
        ...
        configuration = dual_galt.calculate_bit_loc_galt(False, configuration)
    elif configuration['REPLICATED_STRUCT']:
        ...
        if llink['DIR'] == localdir:
            ...
            if llink['HASREADY']:
                for rstruct_iteration in list (range (0, 4)):
                    global_struct.g_concat_code_vector_master_tx.append ( sprint_verilog_logic_line (gen_llink_concat_credit (llink['NAME'],localdir)+"_r"+str(rstruct_iteration)) )
        ...
        if llink['DIR'] == localdir:
            if llink['HASVALID']:
                ...
                if llink['NAME'] == "ST_M2S":
                    ...
                    #global_struct.g_concat_code_vector_master_tx.append ( sprint_verilog_assign ((gen_llink_concat_pushbit (llink['NAME'],otherdir)),gen_llink_concat_pushbit (llink['NAME'],otherdir)+"_r"+str(rstruct_iteration)))
                    global_struct.g_concat_code_vector_master_tx.append ( "  assign {:20} = ".format(gen_llink_concat_pushbit (llink['NAME'],otherdir)))
                    for rstruct_iteration in list (range (0, configuration['RSTRUCT_MULTIPLY_FACTOR'])):
                        global_struct.g_concat_code_vector_master_tx.append ( "{:20}".format(gen_llink_concat_pushbit (llink['NAME'],otherdir)+"_r"+str(rstruct_iteration)) )
                ...
                if llink['NAME'] == "ST_S2M":
                    for rstruct_iteration in list (range (0, configuration['RSTRUCT_MULTIPLY_FACTOR'])):
                        global_struct.g_concat_code_vector_slave_rx.append ( "  assign {:20} = ".format(gen_llink_concat_pushbit (llink['NAME'],localdir)+"_r"+str(rstruct_iteration)))
                        global_struct.g_concat_code_vector_slave_rx.append ( "{:20} ;\n".format(gen_llink_concat_pushbit (llink['NAME'],localdir)))
            ...
            if llink['HASREADY']:
                ...
                for rstruct_iteration in list (range (0, 4)):
                    if rstruct_iteration < configuration['RSTRUCT_MULTIPLY_FACTOR'] and localdir == "input":
                        ...
                        if llink['NAME'] == "ST_S2M":
                            global_struct.g_concat_code_vector_master_tx.append ( sprint_verilog_assign (gen_llink_concat_credit (llink['NAME'],localdir)+"_r"+str(rstruct_iteration), (gen_llink_concat_credit (llink['NAME'],localdir)), index2=gen_index_msb(1, rstruct_iteration) ))
                        else:
                            global_struct.g_concat_code_vector_master_rx.append ( sprint_verilog_assign (gen_llink_concat_credit (llink['NAME'],localdir), "1'b0", index1=gen_index_msb(1, rstruct_iteration)   ))
                ...
                for rstruct_iteration in list (range (0, 4)):
                    if rstruct_iteration < configuration['RSTRUCT_MULTIPLY_FACTOR']:
                       if llink['NAME'] == "ST_S2M":
                           global_struct.g_concat_code_vector_slave_tx.append ( sprint_verilog_assign ((gen_llink_concat_credit (llink['NAME'],otherdir)),gen_llink_concat_credit (llink['NAME'],otherdir)+"_r"+str(rstruct_iteration),  index1=gen_index_msb(1, rstruct_iteration) ))
                       else:
                    ...
                    #    global_struct.g_concat_code_vector_slave_tx.append ( sprint_verilog_assign (gen_llink_concat_credit (llink['NAME'],otherdir)+"_r"+str(rstruct_iteration), "|"+(gen_llink_concat_credit (llink['NAME'],otherdir)) ))
                        ...
                        if llink['NAME'] == "ST_S2M":
                            global_struct.g_concat_code_vector_slave_tx.append ( sprint_verilog_assign (gen_llink_concat_credit (llink['NAME'],otherdir), "1'b0",  index1=gen_index_msb(1, rstruct_iteration)) )
                        else:
    ...
    for direction in ['master', 'slave']:
        ...
        for llink in configuration['LL_LIST']:
           ...
           #
                ...
                file_name.write("\n  // {0} channel\n".format(llink['NAME']))
                for sig in llink['SIGNALLIST_MAIN']:
                    if sig['TYPE'] == "rstruct_enable" and sig['DIR'] == "output" and direction == 'master' :  ## Drop the user_enable if in master (slave only)
                        continue
                    if sig['TYPE'] == "rstruct_enable" and sig['DIR'] == "input" and direction == 'slave' :  ## Drop the user_enable if in master (slave only)
                        continue
        ...
        for llink in configuration['LL_LIST']:
            ...
            otherdir = gen_direction(name_file_name, llink['DIR'], False)
          ...
          if localdir == 'output':
              ...
              for rstruct_iteration in list (range (0, configuration['RSTRUCT_MULTIPLY_FACTOR'])):
                  for sig in llink['SIGNALLIST_MAIN']:
                      ...
                      #if sig['TYPE'] == 'rstruct_enable' and localdir == 'input':
                      #    if llink['NAME'] == "ST_S2M":
                      #        print_verilog_assign(file_name, sig['NAME'], gen_llink_user_fifoname (llink['NAME'], otherdir), index1=gen_index_msb(sig['SIGWID'], sig['LSB'] + rstruct_iteration) , index2=gen_index_msb (sig['SIGWID'], (sig['LLINDEX_MAIN_LSB'] * configuration['RSTRUCT_MULTIPLY_FACTOR']) + rstruct_iteration))
                      #    else:
                      #        print_verilog_assign(file_name, sig['NAME'], gen_llink_user_fifoname (llink['NAME'], localdir), index1=gen_index_msb(sig['SIGWID'], sig['LSB'] + rstruct_iteration) , index2=gen_index_msb (sig['SIGWID'], (sig['LLINDEX_MAIN_LSB'] * configuration['RSTRUCT_MULTIPLY_FACTOR']) + rstruct_iteration))
                ...
                for sig in llink['SIGNALLIST_MAIN']:
                    if sig['TYPE'] == "rstruct_enable":
                        ...
                        for rstruct_iteration in list (range (0, configuration['RSTRUCT_MULTIPLY_FACTOR'])):
                            if direction == "master":
                                if llink['NAME'] == "ST_S2M":
                                    file_name.write("  assign {0:20}[{1:4}] = {2};\n".format(gen_llink_concat_fifoname (llink['NAME'],"output" ), llink_lsb, gen_llink_concat_pushbit (llink['NAME'],"output")+"_r"+str(rstruct_iteration) ))
                                    llink_lsb += 1
                            else:
                                if llink['NAME'] == "ST_M2S":
                                    ...
                                    llink_lsb += 1
                            #if llink['NAME'] == "ST_M2S" and direction == 'slave':
                            #    file_name.write("  assign {0:20}[{1:4}] = {2};\n".format(gen_llink_concat_fifoname (llink['NAME'],"output" ), llink_lsb, gen_llink_concat_pushbit (llink['NAME'],llink['DIR'])+"_r"+str(rstruct_iteration) ))
                            #    llink_lsb += 1
                            #else:
                            #    file_name.write("  assign {0:20}[{1:4}] = {2};\n".format(gen_llink_concat_fifoname (llink['NAME'],"input" ), llink_lsb, gen_llink_concat_pushbit (llink['NAME'],llink['DIR'])+"_r"+str(rstruct_iteration) ))
                            #    llink_lsb += 1
    ...
    for direction in ['master', 'slave']:
        ...
        for llink in configuration['LL_LIST']:
           ...
           #
                ...
                file_name.write("\n  // {0} channel\n".format(llink['NAME']))
                for sig_gen2 in llink['SIGNALLIST_MAIN']:
                    if sig_gen2['TYPE'] == "rstruct_enable" and direction == 'master' and llink['NAME'] ==  "ST_M2S":
                        continue
                    if sig_gen2['TYPE'] == "rstruct_enable" and direction == 'slave' and llink['NAME'] ==  "ST_S2M":
                        continue
                    localdir = gen_direction(name_file_name, sig_gen2['DIR'])
                    if sig_gen2['TYPE'] == "rstruct_enable" or configuration['GEN2_AS_GEN1_EN'] == True:
                        ...
                        if llink['NAME'] == "ST_S2M" or llink['NAME'] == "ST_M2S":
                            print_verilog_io_line(file_name, localdir, sig_gen2['NAME'], index=gen_index_msb(sig_gen2['SIGWID'] * configuration['RSTRUCT_MULTIPLY_FACTOR'],sig_gen2['LSB'], sysv=False))
                        #elif llink['NAME'] == "ST_M2S" and direction == 'slave':
                        #    print_verilog_io_line(file_name, localdir, sig_gen2['NAME'], index=gen_index_msb(sig_gen2['SIGWID'] * configuration['RSTRUCT_MULTIPLY_FACTOR'],sig_gen2['LSB'], sysv=False))
                        elif configuration['REPLICATED_STRUCT']:
                            print_verilog_io_line(file_name, localdir, sig_gen2['NAME'], index=gen_index_msb(llink['WIDTH_RX_RSTRUCT'] * configuration['RSTRUCT_MULTIPLY_FACTOR'], sig_gen2['LSB'], sysv=False))
                        else:
                            print_verilog_io_line(file_name, localdir, sig_gen2['NAME'], index=gen_index_msb(sig_gen2['SIGWID'] * configuration['RSTRUCT_MULTIPLY_FACTOR'], sig_gen2['LSB'], sysv=False))
        ...
        for llink in configuration['LL_LIST']:
            ...
            #print("KR_INDEX: WIDTH_RX_RSTRUCT = {} & WIDTH_MAIN = {} & FACTOR = {} ".format(llink['WIDTH_RX_RSTRUCT'],llink['WIDTH_MAIN'],configuration['RSTRUCT_MULTIPLY_FACTOR']))
        ...
        for llink in configuration['LL_LIST']:
            if llink['HASVALID_NOREADY_REPSTRUCT']:
                ...
                if llink['DIR'] == localdir:
                    print_verilog_assign(file_name, "tx_{0}_data".format(llink['NAME']), "txfifo_{0}_data".format(llink['NAME']), index1=gen_index_msb (llink['WIDTH_RX_RSTRUCT']       * configuration['RSTRUCT_MULTIPLY_FACTOR']), index2=gen_index_msb (llink['WIDTH_RX_RSTRUCT'] * configuration['RSTRUCT_MULTIPLY_FACTOR']))
                ...
                if llink['DIR'] == localdir:
                    if configuration['REPLICATED_STRUCT']:
                        if llink['NAME'] == "ST_M2S" and configuration['GEN2_AS_GEN1_EN'] != True:
                            file_name.write("      ll_transmit #(.WIDTH({1}), .DEPTH(8'd{2}), .TX_CRED_SIZE(3'h{3}), .ASYMMETRIC_CREDIT(1'b1), .DEFAULT_TX_CRED(8'd{4})) ll_transmit_i{0}\n".format(llink['NAME'], llink['WIDTH_RX_RSTRUCT'] * configuration['RSTRUCT_MULTIPLY_FACTOR'], (int(llink['RX_FIFO_DEPTH']) + configuration['RSTRUCT_MULTIPLY_FACTOR'] - 1) // configuration['RSTRUCT_MULTIPLY_FACTOR'], configuration['RSTRUCT_MULTIPLY_FACTOR'], llink['RX_FIFO_DEPTH']))
                        else:
                    ...
                    if llink['NAME'] == "ST_M2S" and configuration['GEN2_AS_GEN1_EN'] != True and configuration['REPLICATED_STRUCT']:
        ...
        for llink in configuration['LL_LIST']:
            for sig in llink['SIGNALLIST_MAIN']:
                if sig['TYPE'] == "rstruct_enable" and localdir == "output" and llink['NAME'] == "ST_M2S":
                    continue
                if sig['TYPE'] == "rstruct_enable" and localdir == "input" and llink['NAME'] == "ST_S2M":
                    continue
                ...
                elif llink['NAME'] == "ST_S2M" or llink['NAME'] == "ST_M2S":
                    file_name.write("         .{2:30}   ({2}{1}),\n".format(localdir, gen_index_msb(sig['SIGWID'] * configuration['RSTRUCT_MULTIPLY_FACTOR'], sig['LSB'], sysv=False), sig['NAME']))
                #elif llink['NAME'] == "ST_M2S" and direction == "slave":
                #    file_name.write("         .{2:30}   ({2}{1}),\n".format(localdir, gen_index_msb(sig['SIGWID'] * configuration['RSTRUCT_MULTIPLY_FACTOR'], sig['LSB'], sysv=False), sig['NAME']))
                elif configuration['REPLICATED_STRUCT']:
                    file_name.write("         .{2:30}   ({2}{1}),\n".format(localdir, gen_index_msb(llink['WIDTH_RX_RSTRUCT'] * configuration['RSTRUCT_MULTIPLY_FACTOR'], sig['LSB'], sysv=False), sig['NAME']))
                else:
                    file_name.write("         .{2:30}   ({2}{1}),\n".format(localdir, gen_index_msb(sig['SIGWID'] * configuration['RSTRUCT_MULTIPLY_FACTOR'], sig['LSB'], sysv=False), sig['NAME']))
        ...
        for llink in configuration['LL_LIST']:
            ...
            elif configuration['REPLICATED_STRUCT'] and localdir == "input":
                file_name.write("         .{0:30}   ({0}{1}),\n".format("{}fifo_{}_data".format(prefix,llink['NAME']), gen_index_msb(llink['WIDTH_RX_RSTRUCT'] * configuration['RSTRUCT_MULTIPLY_FACTOR'], sysv=False)))
            elif configuration['REPLICATED_STRUCT']:
                #file_name.write("         .{0:30}   ({0}{1}),\n".format("{}fifo_{}_data".format(prefix,llink['NAME']), gen_index_msb(llink['WIDTH_MAIN'] * configuration['RSTRUCT_MULTIPLY_FACTOR'], sysv=False)))
                file_name.write("         .{0:30}   ({0}{1}),\n".format("{}fifo_{}_data".format(prefix,llink['NAME']), gen_index_msb(llink['WIDTH_RX_RSTRUCT'] * configuration['RSTRUCT_MULTIPLY_FACTOR'], sysv=False)))
        ...
        for llink in configuration['LL_LIST']:
            if llink['DIR'] == localdir:
                ...
                if configuration['REPLICATED_STRUCT'] and configuration['GEN2_AS_GEN1_EN'] != True:
...
def make_list_files(configuration):
    ...
    for direction in ['master', 'slave']:
        ...
        if "PROJ_INT" in os.environ: 
            file_name.write("-f ${PROJ_DIR}/../ip/llink/rtl/llink.f\n")
            file_name.write("\n")
            file_name.write("// Common Files\n")
            file_name.write("-f ${PROJ_DIR}/../ip/common/rtl/common.f\n")
        else:
