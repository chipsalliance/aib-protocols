...
cd cfg_list;
python ca_gen_cfg.py    ## Creating Multiple random sailrock cfg files
cd ~-;
    ...
    DIR=${DIR/_sailrock_cfg.txt/}
       ...
       rm temp.txt
        if  [ ! -f ${DIR}/ca_tb.log ] ;  then
          echo ERROR: $DIR   $testname   did not run
          echo ERROR: $DIR   $testname   did not run. >> regression_results.txt
        elif `grep -qE "xmsim: \*E,ASRTST" ${DIR}/ca_tb.log`; then
          echo ERROR: ${DIR}   $testname   Assertion Failure
          echo ERROR: ${DIR}   $testname   Assertion Failure. >> regression_results.txt
        elif `grep -qE "errors with the code \*E" ${DIR}/ca_tb.log` || `grep -qE "\*E" ${DIR}/ca_tb.log` ; then
          echo ERROR: ${DIR}   $testname   compilation errors
          echo ERROR: ${DIR}   $testname   compilation errors. >> regression_results.txt
        elif `grep -qE "TRNULLID: NULL pointer dereference" ${DIR}/ca_tb.log` ; then
          echo ERROR: ${DIR}   $testname   NULL POINTER errors
          echo ERROR: ${DIR}   $testname   NULL POINTER errors. >> regression_results.txt
        elif ! `grep -qE "UVM_ERROR\s*:\s*0" ${DIR}/ca_tb.log` || ! `grep -qE "UVM_FATAL\s*:\s*0" ${DIR}/ca_tb.log`;  then
          echo ERROR: $DIR   $testname   did not finish
          echo ERROR: $DIR   $testname   did not finish. >> regression_results.txt
        else
          echo pass:  ${DIR}   $testname   completed successfully
          echo pass:  ${DIR}   $testname   completed successfully. >> regression_results.txt
        fi
done
